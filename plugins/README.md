## Required Plugins

- [Citizens](https://ci.citizensnpcs.co/job/Citizens2/)
- [Denizen](https://ci.citizensnpcs.co/job/Denizen_Developmental/)
- [Depenizen](https://ci.citizensnpcs.co/job/Depenizen/)
- [LuckPerms](https://luckperms.net/download)
  - The server scripts are plugin-agnostic and only require that any permissions plugin is present alongside Vault. However, the LuckPerms folder is already fitted with permission data.
- [Sentinel](https://ci.citizensnpcs.co/job/Sentinel/)
- [Vault](https://www.spigotmc.org/resources/vault.34315/)